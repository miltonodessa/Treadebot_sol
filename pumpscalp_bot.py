...
# 15 = C: строгий фильтр — входим только в очень горячие токены (≥15 покупателей за 15 секунд)
PREENTRY_BUYERS_MIN  = int(os.getenv("PREENTRY_BUYERS_MIN", "15"))
...
SYMPATHY_ENABLED          = bool(int(os.getenv("SYMPATHY_ENABLED", "0")))  # A: отключено по умолчанию (70-80% убыточных сделок)
...
class NewTokenStrategy(Strategy):
    ...
    async def on_create(self, event: "TokenEvent") -> None:
        ...
        if PREENTRY_BUYERS_MIN > 0:
            ...
            # B: не более 1 позиции/наблюдения на один символ одновременно
            norm_sym = event.symbol.upper()
            if any(p.symbol.upper() == norm_sym for p in state.positions.values()):
                state.signals_skipped += 1
                log.debug("⛔ B-filter: %s — уже есть позиция с этим символом", event.symbol)
                return
            if any(w['event'].symbol.upper() == norm_sym for w in self.watching.values()):
                state.signals_skipped += 1
                log.debug("⛔ B-filter: %s — уже наблюдаем токен с этим символом", event.symbol)
                return
    ...
    async def on_trade(self, mint: str, msg: dict) -> None:
        ...
        if is_buy and mint in self.watching:
            ...
            if w['buyers'] >= PREENTRY_BUYERS_MIN:
                # B: финальная проверка — символ мог занять другой токен пока мы ждали
                norm_sym = w['event'].symbol.upper()
                if any(p.symbol.upper() == norm_sym for p in state.positions.values()):
                    log.debug("⛔ B-filter: %s — символ занят другой позицией, пропускаем", w['event'].symbol)
